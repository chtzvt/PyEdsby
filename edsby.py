...
class Edsby(object):
    ...
    def getSession(self):
        return requests.Session().get(
            'https://' + self.edsbyHost + "/core/login/3472?xds=loginform&editable=true",
    ...
    def getauthData(self, loginData):
        self.authData = requests.get(
            'https://' + self.edsbyHost + "/core/node.json/3472?xds=fetchcryptdata&type=Plaintext-LeapLDAP",
    ...
    def sendAuthenticationData(self):
        studentData = requests.post(
            'https://' + self.edsbyHost + '/core/login/3472?xds=loginform&editable=true',
            data=self.authData, cookies=self.getCookies(), headers=self.getHeaders()).json()
        print(studentData)
        return {
